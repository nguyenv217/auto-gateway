...
class FailingAliasProvider(BaseProvider):
    """Provider that always fails when called with a specific alias key."""
    def __init__(self, name: str, keys: list[str] | None = None, models: dict[str, list[str]] | None = None, key_aliases: dict[str, str] | None = None):
        super().__init__(name=name, keys=keys, models=models or {"m": []}, key_aliases=key_aliases)
        self.fail_key: str | None = None

    async def call(self, *, key, model, messages, timeout, tools, tool_choice, extra_body=None) -> ProviderCallResult:
        if self.fail_key and key == self.fail_key:
            raise RuntimeError(f"intentional fail for key {key}")
        last_user = ""
        for m in reversed(messages):
            if m.get("role") == "user":
                c = m.get("content")
    ...
    async def call_stream(self, *, key, model, messages, timeout, tools, tool_choice, extra_body=None):
        if self.fail_key and key == self.fail_key:
            raise RuntimeError(f"intentional stream fail for key {key}")
        result = await self.call(key=key, model=model, messages=messages, timeout=timeout, tools=tools, tool_choice=tool_choice, extra_body=extra_body)
        for ch in result["text"]:
            yield {"type": "content", "content": ch}
        yield {"type": "finish", "finish_reason": "stop"}
...
def test_get_keys_for_alias_not_found():
    prov = EchoKeyProvider(name="p", keys=["k1", "k2"], models={"m": []}, key_aliases={"a": "k1"})
...
async def test_router_with_alias_not_found_skips_provider():
    """When alias doesn't match, get_keys_for_alias returns [] and provider is skipped -> AllProvidersExhaustedError."""
    prov = EchoKeyProvider(name="only", keys=["k1"], models={"m": []}, key_aliases={"good": "k1"})
    providers = {"only": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = SequentialStrategy(providers, all_models)
    router = ProviderRouter(providers)
    ...
    req = RouteRequest(
        strategy=strategy, provider=None, alias="bad_alias", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
        messages=[{"role": "user", "content": "hi"}],
    )
...
async def test_router_unrecognized_provider_falls_through_to_all():
    """Unrecognized provider name falls through to all providers (pre-existing behavior)."""
    ...
    req = RouteRequest(
        strategy=strategy, provider="nonexistent_provider", alias=None, models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
        messages=[{"role": "user", "content": "hi"}],
    )
    result = await router.route(req)
...
async def test_router_without_alias_rotates_all_keys():
    ...
    prov = EchoKeyProvider(name="echoer", keys=["k1", "k2"], models={"m": []}, key_aliases={"a": "k1", "b": "k2"})
    providers = {"echoer": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = SequentialStrategy(providers, all_models)
    router = ProviderRouter(providers)
    ...
    req = RouteRequest(
        strategy=strategy, provider=None, alias=None, models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
        messages=[{"role": "user", "content": "hi"}],
    )
    result = await router.route(req)
...
# ── Alias failover tests ──

@pytest.mark.asyncio
async def test_alias_failover_within_same_provider_pins_to_one_key():
    """When alias is specified within a single provider, only that ONE key is tried.
    If it fails, the router does NOT fallback to other keys in the same provider
    because alias pins to a specific key. It returns the error result (no other candidates)."""
    prov = FailingAliasProvider(
        name="failing",
        keys=["k1", "k2"],
        models={"m": []},
        key_aliases={"a": "k1", "b": "k2"},
    )
    prov.fail_key = "k1"  # alias "a" maps to k1 which fails

    providers = {"failing": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = SequentialStrategy(providers, all_models)
    router = ProviderRouter(providers)

    req = RouteRequest(
        strategy=strategy, provider=None, alias="a", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
        messages=[{"role": "user", "content": "fail-test"}],
    )
    result = await router.route(req)
    # Router returns the 'last' default empty result, not AllProvidersExhaustedError,
    # because it attempted the provider but it failed
    assert result["text"] is None


@pytest.mark.asyncio
async def test_alias_failover_across_providers():
    """When the alias matches keys on multiple providers and the first one fails,
    the router falls back to the next provider that also has the same alias."""
    prov1 = FailingAliasProvider(name="prov1", keys=["k1-fail"], models={"m": []}, key_aliases={"shared": "k1-fail"})
    prov1.fail_key = "k1-fail"

    prov2 = EchoKeyProvider(name="prov2", keys=["k2-ok"], models={"m": []}, key_aliases={"shared": "k2-ok"})
    ...
    req = RouteRequest(
        strategy=strategy, provider=None, alias="shared", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
        messages=[{"role": "user", "content": "cross-proxy"}],
    )
    result = await router.route(req)
    assert "prov2" in result["text"]
    assert "k2-ok" in result["text"]


@pytest.mark.asyncio
async def test_alias_failover_across_providers_streaming():
    """Streaming failover: when alias matches keys on multiple providers
    and the first one fails before emitting, the next provider is tried."""
    prov1 = FailingAliasProvider(name="prov1", keys=["k1-fail"], models={"m": []}, key_aliases={"shared": "k1-fail"})
    prov1.fail_key = "k1-fail"
    ...
    async with AsyncClient(base_url="http://test", transport=transport) as client:
        resp = await client.post("/v1/chat/completions", json={
            "model": "m", "messages": [{"role": "user", "content": "stream-failover"}],
            "stream": True, "alias": "shared",
        })
    assert resp.status_code == 200
    full_text = ""
    for ln in resp.text.splitlines():
        if ln.startswith("data: {") and not ln.strip().endswith("[DONE]"):
            chunk = json.loads(ln[len("data: "):])
            delta = chunk["choices"][0]["delta"]
            if delta.get("content"):
                full_text += delta["content"]
    assert "prov2" in full_text
    assert "k2-ok" in full_text
...
async def test_api_with_provider_and_alias():
    prov = EchoKeyProvider(name="apiprov", keys=["kk-east", "kk-west"], models={"m": []}, key_aliases={"east": "kk-east", "west": "kk-west"})
    ...
    async with AsyncClient(base_url="http://test", transport=transport) as client:
        resp = await client.post("/v1/chat/completions", json={
            ...
            "stream": False, "provider": "apiprov", "alias": "west",
        })
    assert resp.status_code == 200
    data = resp.json()
...
async def test_api_with_provider_only():
    prov = EchoKeyProvider(name="apiprov2", keys=["kA", "kB"], models={"m": []}, key_aliases={"a": "kA", "b": "kB"})
    ...
    async with AsyncClient(base_url="http://test", transport=transport) as client:
        resp = await client.post("/v1/chat/completions", json={
            "model": "m", "messages": [{"role": "user", "content": "hello"}],
            "stream": False, "provider": "apiprov2",
        })
    assert resp.status_code == 200
    data = resp.json()
...
async def test_api_with_alias_without_provider():
    ...
    prov1 = EchoKeyProvider(name="prov1", keys=["bad-key"], models={"m": []}, key_aliases={"target": "good-key"})
    prov2 = EchoKeyProvider(name="prov2", keys=["other"], models={"m": []}, key_aliases={})
    providers = {"prov1": prov1, "prov2": prov2}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = SequentialStrategy(providers, all_models)
    router = ProviderRouter(providers)
...
async def test_streaming_with_alias():
    prov = EchoKeyProvider(name="streamprov", keys=["sk-east", "sk-west"], models={"m": []}, key_aliases={"east": "sk-east", "west": "sk-west"})
    ...
    async with AsyncClient(base_url="http://test", transport=transport) as client:
        resp = await client.post("/v1/chat/completions", json={
            "model": "m", "messages": [{"role": "user", "content": "data"}],
            "stream": True, "provider": "streamprov", "alias": "east",
        })
    assert resp.status_code == 200
    full_text = ""
    for ln in resp.text.splitlines():
...
async def test_adaptive_strategy_respects_alias():
    prov = EchoKeyProvider(name="adaptiveprov", keys=["ak-east", "ak-west"], models={"m": []}, key_aliases={"east": "ak-east", "west": "ak-west"})
    providers = {"adaptiveprov": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = AdaptiveStrategy(providers, all_models)
    router = ProviderRouter(providers)
    req = RouteRequest(
        strategy=strategy, provider="adaptiveprov", alias="east", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
...
async def test_bandit_strategy_respects_alias():
    prov = EchoKeyProvider(name="banditprov", keys=["bk-east", "bk-west"], models={"m": []}, key_aliases={"east": "bk-east", "west": "bk-west"})
    providers = {"banditprov": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = UCBBanditStrategy(providers, all_models)
    router = ProviderRouter(providers)
    req = RouteRequest(
        strategy=strategy, provider="banditprov", alias="east", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
...
async def test_sequential_strategy_respects_alias():
    prov = EchoKeyProvider(name="seqprov", keys=["sk-east", "sk-west"], models={"m": []}, key_aliases={"east": "sk-east", "west": "sk-west"})
    providers = {"seqprov": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = SequentialStrategy(providers, all_models)
    router = ProviderRouter(providers)
    req = RouteRequest(
        strategy=strategy, provider="seqprov", alias="west", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
...
async def test_dict_key_with_none_values():
    ...
    prov = EchoKeyProvider(name="nprov", keys=["real-key", None], models={"m": []}, key_aliases={"valid": "real-key"})
    providers = {"nprov": prov}
    all_models = {pn: p.get_all_models() for pn, p in providers.items()}
    strategy = SequentialStrategy(providers, all_models)
    router = ProviderRouter(providers)
    req = RouteRequest(
        strategy=strategy, provider=None, alias="valid", models=["m"],
        timeout=15.0, shuffle=False, tools=None, tool_choice=None, extra_body={},
...
def test_provider_with_empty_aliases_dict():
    ...
    prov = EchoKeyProvider(name="emptyaliases", keys=["k1"], models={"m": []}, key_aliases={})
